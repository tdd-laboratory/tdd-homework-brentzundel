...
_date_ddMonthYear_pat = _whole_word(r'([1-9]|[12][0-9]|3[01]) (Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) \d{4}')
...
def dates_iso8601(text):
    '''find 'yyyy-mm-dd' formatted dates in text.'''
    for match in _date_iso8601_pat.finditer(text):
        yield('date', match)

def dates_ddMonYear(text):
    '''find 'dd Month Year' formatted dates in text'''
    for match in _date_ddMonthYear_pat.finditer(text):
        yield('date', match)